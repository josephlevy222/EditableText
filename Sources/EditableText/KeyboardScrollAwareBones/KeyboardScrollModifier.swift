...
struct KeyboardScrollModifier: ViewModifier {
	...
	@ObservedObject var focusID = FocusCoordinator.shared
...
class FocusCoordinator: ObservableObject {
	static let shared = FocusCoordinator() // The Singleton instance
	static let focusCoordinator = shared.activeID
	@Published var activeID: String?
	// Private init ensures no one else can create a new focusID
	private init() {}
}

struct KeyboardFieldModifier: ViewModifier {
	@ObservedObject var focusID = FocusCoordinator.shared
	var activeID: String? { get { focusID.activeID } set { focusID.activeID = newValue}}
...
extension View {
	public func focusKeyboardAware() -> some View {
